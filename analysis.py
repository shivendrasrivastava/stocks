...
def get_portfolio_value(prices, allocs, start_val=1):
    ...
    # Create a temporary data frame to compute portfolio value
    # first we assign the normalised prices to the data frame
    df = prices/prices.iloc[0]
    # next we multiply the normalized prices by the allocation to each stock
    df = df * allocs
    # next we multiply all cells by the starting value, the previous step ensures this allocates proportionally
    df = df * start_val
    # now we sum rows to get a datframe showing the daily portfolio value.
...
def get_portfolio_stats(port_val, daily_rf=0, samples_per_year=252):
    ...
    # Calculate the cumulative return over the period by dividing the last bye the 
    # first row of port_val and taking away 1
    cum_ret = (port_val[-1] - port_val[0]) / port_val[0]
    
    # Now we use a function to calculate the daily percentage change
    # as there is no change on the first day we can throw this row out.
    port_val_chg = port_val.pct_change()[1:]
    # We use this data frame to calculate the next statistics
    avg_daily_ret = port_val_chg.mean()
    std_daily_ret = port_val_chg.std()
...
def test_run2():
    ...
    end_date = '2010-12-31' 
    symbol_allocations = OrderedDict([('GOOG', 0.2), ('AAPL', 0.3), ('GLD', 0.4), ('XOM', 0.1)])  # allocations from wiki example
